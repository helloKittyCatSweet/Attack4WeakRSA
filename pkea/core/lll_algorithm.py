...
class ImprovedLLL:
    ...
    Implements the Lenstra-Lenstra-Lovász algorithm for lattice basis reduction.
    Used in cryptanalysis to find short vectors in lattices.
    """

    def __init__(self, delta: float = 0.99):
        """
        Initialize LLL algorithm
        
        Args:
            delta: Reduction parameter (0.25 < delta < 1). Higher values give better reduction.
                   Default 0.99 for better success rate in cryptanalysis.
        """
        self.delta = delta
    ...
    def gram_schmidt(self, B: List[List[int]]) -> Tuple[List[List[float]], List[List[float]]]:
        ...
        Args:
            B: Lattice basis matrix (list of row vectors)
            
        Returns:
            Tuple of (B_star, mu) where:
            - B_star: Orthogonalized basis
    ...
    def get_shortest_vector(self, lattice: List[List[int]]) -> List[int]:
        ...
        Args:
            lattice: Lattice basis matrix
            
        Returns:
            Shortest non-zero vector in the reduced lattice
        """
